            ...
            <table class="table table-hover text-center">
                ...
                <tr>
                    <form action="${pageContext.request.contextPath}/operative/updatesite" method="get" id="user<%=i%>">
                        <td><%=siteBeans.get(i).getId()%>
                        </td>
                        <td><%=siteBeans.get(i).getSite()%>
                        </td>
                        <td><%=siteBeans.get(i).getStatus()%>
                        </td>
                        <td><input type="text" name="status"/><input type="text" name="id" hidden
                                                                     value="<%=siteBeans.get(i).getId()%>"/></td>
                        <td>
                            <div class="button-group"><a class="button border-red"
                                                         onclick="document.getElementById('user<%=i%>').submit();return false"><span
                                    class="icon-trash-o"></span> 修改</a></div>
                        </td>
                        <td>
                            <div class="button-group"><a class="button border-red"
                                                         href="${pageContext.request.contextPath}/operative/delsite?id=<%=siteBeans.get(i).getId()%>"><span
                ...
                <tr>
                    <td colspan="8">
                        <div class="pagelist">
                            ...
                            <%} else {%>
                            <a href="${pageContext.request.contextPath}/operative/site?page=<%=i%>"><%=i%>
                            </a>
                            <%
                                    }
                                }
