...
<%@ page import="com.fk.bean.ReturnLineAndOrders" %>
<%--
    ...
    <div class="panel admin-panel">
        ...
        <div class="body-content">
            <table class="table table-hover text-center">
                ...
                <%
                    List<ReturnLineAndOrders> users = (List) request.getAttribute("list");
                    for(int i=0;i<users.size();i++){
                %>
                <tr>
                    <td><%=users.get(i).getLineId()%></td>
                    <td><%=users.get(i).getLineTitle()%></td>
                    <td><%=users.get(i).getLinesummary()%></td>
                    <td><%=users.get(i).getOrderId()%></td>
                    <td><%=users.get(i).getOrderTitle()%></td>
                    <td><div class="button-group"> <a class="button border-red" href="${pageContext.request.contextPath}/operative/delline?id=<%=users.get(i).getLineId()%>"><span class="icon-trash-o"></span> 删除</a> </div></td>
                </tr>
                <%}%>
                <tr>
                    <td colspan="8">
                        <div class="pagelist">
                            ...
                            <%}else{%>
                            <a href="${pageContext.request.contextPath}/operative/der?page=<%=i%>"><%=i%></a>
                            <%}}%>
                            <a href="${pageContext.request.contextPath}/operative/der?page=${pageNow+1}">下一页</a>
                        </div>
                    </td>
                </tr>
            </table>
