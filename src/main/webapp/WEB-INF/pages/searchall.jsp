        ...
        <div class="cont-main _j_feed_list" >
            ...
            <div class="gl-post">
                <div data-cs-t="gonglve_feed" class="_j_feed_data" >
                    ...
                    <%
                        List<Object> travel = (List) request.getAttribute("travel");
                        for(int i=0;i<travel.size();i++){
                            if( travel.get(i) instanceof OrdersBean){
                                OrdersBean ordersBean = (OrdersBean) travel.get(i);
                    %>
                    <div class="feed-item _j_feed_item">
                        <a href="${pageContext.request.contextPath}/shop?id=<%=ordersBean.getId()%>" >
                            <div class="bar clearfix">
                                <span class="stat"><span class="num"><%=ordersBean.getSellcount()%></span> 穷游客买过<i class="icon-hand"></i></span>
                            </div>
                            <div class="title">
                                <%=ordersBean.getTitle()%>
                            </div>
                            <dl class="art clearfix">
                                <dt>
                                    <img src="<%=ordersBean.getImage()%>" width="220" height="150">
                                </dt>
                                <dd>
                                    <div class="info">
                                        <%=ordersBean.getCost()%>
                                    </div>
                                    <span class="art-tag"><%=ordersBean.getSubject()%></span>
                                    <div class="ext-r"><span class="nums"><%=ordersBean.getCount()%>浏览</span></div>
                                </dd>
                            </dl>
                        </a>
                    </div>
                    <div class="hr"></div>
                    <%}else if(travel.get(i) instanceof TravelBean){
                        TravelBean travelBean = (TravelBean) travel.get(i);%>

                    <div class="feed-item _j_feed_item">
                        <a href="${pageContext.request.contextPath}/article?id=<%=travelBean.getId()%>">
                            <div class="bar clearfix">
                                <span class="stat"><span class="num"><%=travelBean.getCount()%></span> 穷游客赞<i class="icon-hand"></i></span>
                            </div>
                            <div class="title">
                                <%=travelBean.getTitle()%>
                            </div>
                            <dl class="art clearfix">
                                <dt>
                                    <img src="<%=travelBean.getImage()%>" width="220" height="150">
                                </dt>
                                <dd>
                                    <div class="info">
                                        <%=travelBean.getSummary()%>
                                    </div>
                                    <%
                                        String[] ty = travelBean.getType().split("\\|");
                                        for(int j=0;j<ty.length;j++){
                                    %>
                                    <span class="art-tag"><%=ty[j]%></span>
                                    <% } %>
                                    <div class="ext-r"><span class="nums"><%=travelBean.getLookcount()%>浏览</span></div>
                                </dd>
                            </dl>
                        </a>
                    </div>
                    ...
                    <div class="drop-pop">
                        <div class="inner">
                            <ul>
                                ...
                                %>
                                <li><a href="${pageContext.request.contextPath}/search?indexsearch=type&key=<%=type.get(i).getType()%>"><%=type.get(i).getType()%>(<%=type.get(i).getCount()%>)</a></li>
                               <%}%>
                            </ul>
                        </div>
                    </div>
