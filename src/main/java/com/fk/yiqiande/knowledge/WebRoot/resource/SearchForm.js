...
var SEARCHFORM = (function(){
	...
	function init(o) {
		...
		$(window).unload(function() { if (browseindexWindow) browseindexWindow.close(); });
	...
	function submitSearch(event) {
		...
		if(searchwords.val().length == 0){
			allEmpty = true;
			searchwords.each(function(){if($(this).val().length > 0 ){allEmpty = false;}});
		}
		if (allEmpty) {
	...
	function enableSearchButton(event) {
		...
		if($(".searchword").val().length == 0){
			allEmpty = true;
			$(".searchword").each(function(){if($(this).val().length > 0 ){allEmpty = false;}});
		}
	...
	function showAdvOptions(show){
		...
		if (!$("#advancedOptions").length) {return;} 
		if (show) {
			//if its already visible don't try to show it again. 
			if($("#advancedOptionshidden").is(":visible")){return;}
